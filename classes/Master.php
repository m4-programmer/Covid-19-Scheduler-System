...
Class Master extends DBConnection {
	...
	/*This handles the deleting of the user's*/
...
// 		die();
		...
		if($save_inv){
			...
			if($save_sched && $save_meta){
				...
				$resp['name'] = $name;
				$resp['code'] = $code;
				// we store the user image here
				if(isset($_FILES['image']) && $_FILES['image']['tmp_name'] != ''){
				$filename = 'uploads/patients/'.$_FILES['image']['name'];
				$move = move_uploaded_file($_FILES['image']['tmp_name'],'../'. $filename);
				$qry = $this->conn->query("UPDATE `individuals` set image = '{$filename}' where id = '{$id}' ");	
			}
			// End of Storing User Image
				// $resp['image'] = $_FILES['image']['name'];
			$resp['image'] = isset($filename) ? $filename : "";
			}else{
				$resp['status'] = 'failed';
